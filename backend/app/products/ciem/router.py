...
@router.post("/scans", response_model=schemas.IdentityScan, status_code=201, tags=["CIEM"])
def trigger_scan(
    db: Session = Depends(dependencies.get_db_from_request),
    current_user: User = Depends(dependencies.get_current_user),
):
    """
    Triggers a new CIEM scan.
    """
    ...
    for finding_data in mock_findings:
        finding_schema = schemas.IdentityFindingCreate(**finding_data)
...
@router.get("/findings", response_model=t.List[schemas.IdentityFinding], tags=["CIEM"])
def list_findings(
    db: Session = Depends(dependencies.get_db_from_request),
    current_user: User = Depends(dependencies.get_current_user),
    skip: int = 0,
    ...
    """
    Retrieve all CIEM findings for the current user's tenant.
    """
